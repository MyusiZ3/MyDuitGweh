...
class FirestoreService {
  ...
  Future<String> createWallet(WalletModel wallet) async {
    ...
    await docRef.set(wallet.copyWith(id: customId, inviteCode: inviteCode).toJson());
    return customId;
  }
  ...
  Future<List<TransactionModel>> getFilteredTransactions({
    ...
    return snapshot.docs.map((doc) => TransactionModel.fromJson(doc.data() as Map<String, dynamic>, docId: doc.id)).toList();
  }

  Stream<List<TransactionModel>> getTransactionsStream(String walletId) {
    return _firestore.collection('transactions').where('walletId', isEqualTo: walletId).orderBy('date', descending: true).snapshots()
        .map((snapshot) => snapshot.docs.map((doc) => TransactionModel.fromJson(doc.data(), docId: doc.id)).toList());
  }
