...
class _UserManagementScreenState extends State<UserManagementScreen> {
  ...
  void _showRoleSelection(String uid, String currentRole, String name) async {
    ...
    final List<String> roleLabels = ['User Biasa', 'Administrator', 'Owner / Super Admin'];
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Atur Kasta ${name.split(' ')[0]}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
            const SizedBox(height: 8),
            Text('Pilih peran baru untuk pengguna ini', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
            const SizedBox(height: 24),
            ...List.generate(roles.length, (index) {
              final isCurrent = currentRole == roles[index];
              return ListTile(
                onTap: isCurrent ? null : () => _updateRole(uid, roles[index], name),
                leading: Container(
                  padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(
                    color: isCurrent ? AppColors.primary.withOpacity(0.1) : Colors.grey[100],
                    shape: BoxShape.circle,
                  ),
                  child: Icon(
                    roles[index] == 'superadmin' ? Icons.shield_rounded : (roles[index] == 'admin' ? Icons.verified_user_rounded : Icons.person_rounded),
                    color: isCurrent ? AppColors.primary : (roles[index] == 'superadmin' ? Colors.indigo : (roles[index] == 'admin' ? Colors.blue : Colors.grey)),
                    size: 20,
                  ),
                ),
                title: Text(roleLabels[index], style: TextStyle(
                  fontWeight: isCurrent ? FontWeight.w900 : FontWeight.normal,
                  color: isCurrent ? AppColors.primary : Colors.black,
                )),
                trailing: isCurrent ? Icon(Icons.check_circle_rounded, color: AppColors.primary) : null,
              );
            }),
  ...
  void _updateRole(String uid, String newRole, String name) async {
    ...
    final roleLabel = newRole == 'superadmin' ? 'Owner' : (newRole == 'admin' ? 'Admin' : 'User Biasa');
    
    final confirm = await UIHelper.showConfirmDialog(
      context: context,
      title: 'Ubah Akses?',
      message: 'Apakah Anda yakin ingin mengubah akses $name menjadi $roleLabel?',
      confirmText: 'YA, UBAH',
      cancelText: 'BATAL',
      isDangerous: false,
    );
    ...
    if (confirm == true) {
      try {
        ...
        if (mounted) {
          _showSuccessSheet('Berhasil!', '$name sekarang memiliki akses $roleLabel.');
        }
      } catch (e) {
        if (mounted) UIHelper.showErrorSnackBar(context, 'Gagal ubah akses: $e');
      }
    }
  }
  ...
  void _confirmToggleStatus(String uid, bool currentStatus, String name) async {
    final confirm = await UIHelper.showConfirmDialog(
      ...
      message: 'Apakah Anda yakin ingin ${currentStatus ? 'mengaktifkan kembali' : 'membekukan'} akun $name?',
    ...
    if (confirm == true) {
      try {
        await _firestore.collection('users').doc(uid).update({'isDeactivated': !currentStatus});
        await _firestore.collection('app_config').doc('global').collection('history').add({
          'updatedBy': _authService.auth.currentUser?.uid ?? 'system',
          'updatedAt': FieldValue.serverTimestamp(),
        ...
        if (mounted) _showSuccessSheet('Status Diperbarui', 'Status akun telah berhasil diubah.');
      } catch (e) {
        if (mounted) UIHelper.showErrorSnackBar(context, 'Gagal ubah status: $e');
      }
    }
  }
  ...
  void _deleteUser(String uid, String name) async {
    final confirm = await UIHelper.showConfirmDialog(
      ...
      message: 'Tindakan ini tidak bisa dibatalkan. Akun $name akan dihapus selamanya dari sistem.',
    ...
    if (confirm == true) {
      try {
        ...
        if (mounted) _showSuccessSheet('User Dihapus', 'Data user telah dibersihkan dari sistem.');
      } catch (e) {
        if (mounted) UIHelper.showErrorSnackBar(context, 'Gagal menghapus user: $e');
      }
    }
  }
                ...
                builder: (context, snapshot) {
                  ...
                  if (users.isEmpty) {
                    return Center(child: Column(
                      ...
                      children: [
                        Icon(Icons.person_off_rounded, color: Colors.grey[300], size: 64),
                        const SizedBox(height: 16),
                        const Text('Tidak ada user ditemukan', style: TextStyle(color: Colors.grey)),
                      ],
                    ));
                  }
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                        ...
                                        child: Container(
                                          ...
                                          child: Text(
                                            ...
                                            style: TextStyle(
                                              ...
                                              color: role == 'superadmin' ? Colors.amber[900] : 
                                                     role == 'admin' ? AppColors.primary : Colors.grey
                                            )
                                          ),
                                        ),
                                      ),
                                  ...
                                  Text('Joined: $joinDateStr', style: TextStyle(color: Colors.grey[400], fontSize: 10)),
                                ],
                              ),
                            ),
      ...
      itemBuilder: (ctx) => [
        if (canManageRole)
          PopupMenuItem(
            ...
            child: Row(
              children: [
                Icon(Icons.theater_comedy_rounded, size: 20, color: Colors.blue[700]),
        ...
        if (canManageStatus)
          PopupMenuItem(
            ...
            child: Row(
              children: [
                Icon(isDeactivated ? Icons.check_circle_rounded : Icons.block_flipped, 
                     size: 20, color: isDeactivated ? Colors.green : Colors.red),
                const SizedBox(width: 12),
                Text(isDeactivated ? 'Aktifkan User' : 'Bekukan Akun', 
                     style: TextStyle(color: isDeactivated ? Colors.green : Colors.red, fontSize: 13)),
              ],
            ),
          ),
        if (canDelete) ...[
          ...
          PopupMenuItem(
            ...
            child: Row(
              children: [
                const Icon(Icons.delete_forever_rounded, size: 20, color: Colors.black),
                const SizedBox(width: 12),
                const Text('Hapus Permanen', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 13)),
              ],
            ),
          ),
        ],
